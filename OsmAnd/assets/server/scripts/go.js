...
var iosAppRedirect = {
	config:{
		...
		containerid:'gocontainer',
		***REMOVED***Name:'OsmAndInstalled',
		***REMOVED***NoExpirationTimeoutInDays:30
	},
	init:function(config){
		if (config && typeof (config) == 'object') {
	...
	setCookie:function(appInstalled){
		if (appInstalled === true){
			Cookies.set(iosAppRedirect.config.***REMOVED***Name, "yes");
		}else{
			Cookies.set(iosAppRedirect.config.***REMOVED***Name, "no", { expires: iosAppRedirect.config.***REMOVED***NoExpirationTimeoutInDays });
		}
	},	
