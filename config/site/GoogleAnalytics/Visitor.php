 ...
 * persistent for the same user permanently (similar to the "__umta" ***REMOVED*** of
 * the GA Javascript client).
 */
class Visitor {
	
	/**
	 * Unique user ID, will be part of the "__utma" ***REMOVED*** parameter
	 * 
	 ...
	 * "currentVisitTime" and "visitCount" properties from the given "__utma" ***REMOVED***
	 * value.
	 * 
	 * @see Internals\ParameterHolder::$__utma
	...
	public function fromUtma($value) {
		$parts = explode('.', $value);
		if(count($parts) != 6) {
			Tracker::_raiseError('The given "__utma" ***REMOVED*** value is invalid.', __METHOD__);
			return $this;
		}
